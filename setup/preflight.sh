...
if [ "`lsb_release -d | sed 's/.*:\s*//' | sed 's/18\.04\.[0-9]/18.04/' `" != "Ubuntu 18.04 LTS" ]; then
	echo "Cakeshop-in-a-Box only supports being installed on Ubuntu 18.04, sorry. You are running:"
	echo
...
if [ ! -d /vagrant ]; then
	...
	echo "Your Cakeshop-in-a-Box needs more memory (RAM) to function properly."
...
if [ $TOTAL_PHYSICAL_MEM -lt 750000 ]; then
	echo "WARNING: Your Cakeshop-in-a-Box has less than 768 MB of memory."
...
if [ -n "$MOUNTED_TMP_AS_NO_EXEC" ]; then
	echo "Cakeshop-in-a-Box has to have exec rights on /tmp, please mount /tmp with exec"
	exit
fi

# Check that no .wgetrc exists
if [ -e ~/.wgetrc ]; then
	echo "Cakeshop-in-a-Box expects no overrides to wget defaults, ~/.wgetrc exists"
	exit
fi
...
if [ "$ARCHITECTURE" != "x86_64" ] && [ "$ARCHITECTURE" != "i686" ]; then
	echo
	echo "WARNING:"
	echo "Cakeshop-in-a-Box has only been tested on x86_64 and i686 platform"
